...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let divBrightnessInput = document.getElementsByClassName(
    ...
    let divBrightnessOutput = document.getElementsByClassName(
    ...
    let buttonGuestModeDisabled = document.getElementById(
        'guest-mode-disabled'
    );

    let divSectionConfig = document.getElementById('section-config');
    ...
    function getConfig() {
        sendRequest('GET', './api/lights/config')
            .then((json) => {
                try {
    ...
    async function setConfig(ip, port) {
        let body = {
            ip: ip,
            port: port,
            guest: buttonGuestModeEnabled.classList.contains('active'),
        };

        return new Promise((resolve, reject) => {
            sendRequest('POST', './api/lights/config', JSON.stringify(body))
                .then(() => {
                    getConfig();
                    resolve();
                })
    ...
    function getStatus() {
        sendRequest('GET', './api/lights/status')
            .then((json) => {
                try {
                    let response = JSON.parse(json);
                    ...
                    if (typeof result !== 'undefined') {
                        ...
                        if (
                            !document
                                .getElementsByClassName(status.color)[0]
                                .classList.contains('active') ||
                            status.mode === '2'
                        ) {
                            for (let i = 0; i < divSwatch.length; i++) {
                                divSwatch[i].parentElement.classList.remove(
                                    'active'
                                );
                                divSwatch[i].classList.remove('active');
                                if (
                                    status.mode === '1' &&
                                    divSwatch[i].classList.contains(
                                        status.color
                                    )
                                ) {
                                    divSwatch[i].parentElement.classList.add(
                                        'active'
                                    );
    ...
    async function setPower(power) {
        ...
        return new Promise((resolve, reject) => {
            sendRequest('POST', './api/lights/power', JSON.stringify(body))
                .then(() => {
                    getStatus();
                    resolve();
                })
    ...
    async function setBrightness(brightness, mode) {
        ...
        return new Promise((resolve, reject) => {
            sendRequest('POST', './api/lights/brightness', JSON.stringify(body))
                .then(() => {
                    getStatus();
                    resolve();
                })
    ...
    function processRadial(posX, posY) {
        if (typeof posX !== 'undefined' && typeof posY !== 'undefined') {
            let centerX =
                divCircle.offsetWidth / 2 +
                divCircle.getBoundingClientRect().left;
            let centerY =
                divCircle.offsetHeight / 2 +
                divCircle.getBoundingClientRect().top;
    ...
    async function setColor(color, mode) {
        ...
        return new Promise((resolve, reject) => {
            sendRequest('POST', './api/lights/color', JSON.stringify(body))
                .then(() => {
                    getStatus();
                    resolve();
                })

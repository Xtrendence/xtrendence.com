...
export async function readCoin({
    token,
    userID,
    assetID,
    assetSymbol,
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (valid) {
                ...
                db.db?.get(
                    'SELECT * FROM Coin WHERE assetID = ?',
                    [id],
                    async (error, row: any) => {
                        if (error) {
                            console.log(error);
                            ...
                            if (
                                row === undefined ||
                                !Utils.validJSON(row.data) ||
                                Utils.refetchRequired(JSON.parse(row.data).time)
                            ) {
                                try {
                                    let from = Math.floor(
                                        Utils.previousYear(
                                    ...
                                    if (!('error' in historicalData)) {
                                        ...
                                        db.runQuery(
                                            'INSERT OR REPLACE INTO Coin (assetID, assetSymbol, data) VALUES (?, ?, ?)',
                                            [id, assetSymbol, data]
                                        );

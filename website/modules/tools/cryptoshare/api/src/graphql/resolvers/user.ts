        ...
        if (settings.userRegistration === 'enabled') {
            return userExists({ username: username })
                .then(async (result) => {
                    if (result === 'Not found.') {
                        if (
                            Utils.validUsername(username) &&
                            Utils.xssValid(username)
                        ) {
                            let keys: any = await Utils.checkKeys();
                            let decryptedPassword = await CryptoFN.decryptRSA(
                                password,
                                keys.privateKey
                            );
...
export async function updateUser({ token, userID, key }: any) {
    try {
        ...
        if (valid) {
            db.runQuery('UPDATE User SET key = ? WHERE userID = ?', [
                key,
                userID,
            ]);
            return 'Done';

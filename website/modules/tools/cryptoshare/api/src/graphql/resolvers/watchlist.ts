...
export async function createWatchlist({
    ...
    try {
        ...
        if (valid) {
            db.runQuery(
                'INSERT INTO Watchlist (userID, assetID, assetSymbol, assetType) VALUES (?, ?, ?, ?)',
                [userID, assetID, assetSymbol, assetType]
            );
            return 'Done';
        } else {
            return 'Unauthorized';
...
export async function updateWatchlist({
    ...
    try {
        ...
        if (valid) {
            db.runQuery(
                'UPDATE Watchlist SET assetID = ?, assetSymbol = ?, assetType = ? WHERE watchlistID = ? AND userID = ?',
                [assetID, assetSymbol, assetType, watchlistID, userID]
            );
            return 'Done';
        } else {
            return 'Unauthorized';

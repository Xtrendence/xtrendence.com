                    ...
                    async (error, row: any) => {
                        if (error) {
                            ...
                            if (
                                ...
                                Utils.refetchRequired(
                                    JSON.parse(row.historicalData).time
                                )
                            ) {
                                try {
                                    ...
                                    if (
                                        ...
                                        reject(
                                            `!${
                                                JSON.parse(historicalData).error
                                            }!`
                                        );
                                        return;
                                    }
        ...
        try {
            ...
            if (valid) {
                try {
                    ...
                    let refetch: any =
                        (await getSymbolsToRefetch(symbols)) || {};
                    let refetchSymbols = Object.keys(refetch);

                    try {
                        if (refetchSymbols.length > 0) {
                            let priceData: any = await getPriceData(
                                refetchSymbols,
                                keyAPI
                            );
                            ...
                            let fetchedSymbols = Object.keys(
                                priceData.priceData
                            );
        ...
        try {
            ...
            for (let i = 0; i < symbols.length; i++) {
                ...
                let priceData: any = await Utils.request(
                    'GET',
                    stockAPI +
                        '/v6/finance/quote?lang=en&symbols=' +
                        joinedSymbols,
                    null,
                    [['X-API-KEY', keyAPI]]
                );

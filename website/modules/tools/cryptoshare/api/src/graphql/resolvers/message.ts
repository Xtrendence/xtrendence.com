...
export async function createMessage({ token, userID, message }: any) {
    try {
        ...
        if (valid) {
            db.runQuery(
                "INSERT INTO Message (userID, message, messageDate) VALUES (?, ?, DATETIME('now'))",
                [userID, message]
            );
            return 'Done';
        } else {
...
export async function updateMessage({
    ...
    try {
        ...
        if (valid) {
            db.runQuery(
                'UPDATE Message SET message = ? WHERE messageID = ? AND userID = ?',
...
export async function deleteMessage({ token, userID, messageID }: any) {
    try {
        ...
        if (valid) {
            db.runQuery(
                'DELETE FROM Message WHERE messageID = ? AND userID = ?',
                [messageID, userID]
            );
            return 'Done';
        } else {

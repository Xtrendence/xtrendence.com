...
export async function createHolding({
    ...
    try {
        ...
        if (valid) {
            db.runQuery(
                'INSERT INTO Holding (userID, holdingAssetID, holdingAssetSymbol, holdingAssetAmount, holdingAssetType) VALUES (?, ?, ?, ?, ?)',
                [
                    userID,
                    holdingAssetID,
                    holdingAssetSymbol,
...
export async function readHolding({ token, userID }: any) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (valid) {
                db.db?.all(
                    'SELECT * FROM Holding WHERE userID = ?',
                    [userID],
                    (error, rows) => {
                        if (error) {
                            console.log(error);
                            ...
                            rows.map((row: any) => {
                                let holding = new Holding(
                                    userID,
                                    row.holdingAssetID,
                                    row.holdingAssetSymbol,
                                    row.holdingAssetAmount,
                                    row.holdingAssetType
                                );
...
export async function updateHolding({
    ...
    try {
        ...
        if (valid) {
            db.runQuery(
                'UPDATE Holding SET holdingAssetID = ?, holdingAssetSymbol = ?, holdingAssetAmount = ?, holdingAssetType = ? WHERE holdingID = ? AND userID = ?',
                [
                    holdingAssetID,
                    holdingAssetSymbol,
                    holdingAssetAmount,
...
export async function deleteHolding({ token, userID, holdingID }: any) {
    try {
        ...
        if (valid) {
            db.runQuery(
                'DELETE FROM Holding WHERE holdingID = ? AND userID = ?',
                [holdingID, userID]
            );
            return 'Done';
        } else {

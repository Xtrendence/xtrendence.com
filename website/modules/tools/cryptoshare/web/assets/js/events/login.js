...
buttonLoginAccount.addEventListener('click', async () => {
    try {
        if (
            empty(inputLoginUsername.value) ||
            empty(inputLoginPassword.value)
        ) {
        ...
        if (
            (empty(inputLoginAPI.value) && appPlatform === 'app') ||
            (empty(inputLoginAPI.value) && appBypass())
        ) {
        ...
        if (appPlatform === 'app' || appBypass()) {
            ...
            try {
                if (
                    !urlAPI.includes('http://') &&
                    !urlAPI.includes('https://')
                ) {
                    urlAPI = `http://${urlAPI}`;
                }
        ...
        login(inputLoginUsername.value, inputLoginPassword.value)
            .then(async (result) => {
                if ('error' in result) {
                    errorNotification(result.error.replaceAll('!', ''));
                } else {
                    socket = io.connect(ip + ':' + port, {
                        path: getPath() + '/socket.io',
                    });
                    attachSocketEvents(socket);

                    let decrypted = CryptoFN.decryptAES(
                        result.key,
                    ...
                    if (currentChoices?.settingsSync === 'disabled') {
                        ...
                        if (!empty(result.settings)) {
                            let decryptedSettings = CryptoFN.decryptAES(

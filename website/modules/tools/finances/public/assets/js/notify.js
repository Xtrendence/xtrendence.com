...
class XNotify {
    ...
    setOptions(options, type) {
        this.content = this.empty(options.content)
            ? this.defaults.content
            : options.content;

        this.maxWidth = this.empty(options.maxWidth)
    ...
    createElement() {
        if (!document.getElementById('x-notify-container')) {
            ...
            container.style =
                'position:fixed; z-index:999; max-width:' +
                (typeof this.maxWidth === 'string' &&
                this.maxWidth.includes('%')
                    ? this.maxWidth
                    : this.maxWidth + 'px') +
                '; width:calc(' +
                this.width +
        ...
        notification.classList.add('glass');
        notification.style =
            'color:' +
            this.color +
            '; width:' +
            ...
            '; border-radius:' +
            this.borderRadius +
            '; font-family:"Helvetica Neue", "Lucida Grande", "Arial", "Verdana", "Tahoma", sans-serif; display:inline-block; text-align:left; opacity:0; pointer-events:auto; -webkit-user-select:none; -khtml-user-select:none; -moz-user-select:none; -ms-user-select:none; user-select:none; outline:none;';
    ...
    generateID() {
        ...
        if (
            this.empty(document.getElementById('x-notify-container').innerHTML)
        ) {
            return id;
        }

...
async function fetchAssets() {
  try {
    ...
    for (const id of ids) {
      ...
      const displayType =
        parsedPrices[item.asset.toLowerCase()]?.typeDisp.toLowerCase() ||
        'other';
    ...
    totalAssets.setAttribute('data-total-cryptocurrency', total.cryptocurrency);
    ...
    for (const inputAsset of document.getElementsByClassName('input-asset')) {
      ...
      if (!Object.keys(aliased).includes(asset) && !asset.includes('-USD'))
        continue;

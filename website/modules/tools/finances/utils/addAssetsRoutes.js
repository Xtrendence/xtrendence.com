...
export function addAssetsRoutes(app, files, intervals, functions) {
  ...
  app.delete('/prices/:asset', async (req, res) => {
    ...
    if (prices[asset]) {
      delete prices[asset];
      delete prices['lastFetched'];
      writeFileSync(files.pricesFile, JSON.stringify(prices, null, 4));
    }

    functions?.refreshPrices();
  ...
  app.get('/financial-assets/:asset', async (req, res) => {
    try {
      ...
      const searches = JSON.parse(readFileSync(files.searchesFile).toString());
      ...
      if (
        ...
        searches[asset.toLowerCase()].fetched + intervals.searchInterval >
          Date.now()
      ) {
      ...
      if (response) {
        ...
        writeFileSync(files.searchesFile, JSON.stringify(searches, null, 4));
      ...
      if (bodyAsset.charAt(0) !== '-') {
        ...
        try {
          ...
          if (
            ...
            (prices[asset.toLowerCase()].fetched + intervals.priceInterval <
              Date.now() &&
              prices['lastFetched'] + intervals.priceDelay < Date.now())
          ) {
            console.log(`Fetching ${asset} price from Yahoo Finance...`);
